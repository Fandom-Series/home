...
up.on("up:link:follow", function(e) {
    ...
    })), YerSelected = document.getElementById("fandomMain").getAttribute("yers"), YerSelected && (ArrayYer = YerSelected.split(","), ArrayYer.forEach(function(e) {
        if (e) {
            if ("2019" == e) var t = 0;
            if ("2018" == e) t = 1;
            if ("2017" == e) t = 2;
            ...
            if ("2006" == e) t = 13;
            if ("2005" == e) t = 14;
            e < "2005" ? (YerasElemts = document.querySelector(".Ayears input"), YerasElemts.setAttribute("value", e), cfilter(YerasElemts, e, "2")) : (YerasElemts = document.querySelector(".Ayears").getElementsByTagName("li")[t], cfilter(YerasElemts, e, 2))
        }
    }))
});
...
function liveSearch(e, t) {
    var i = document.getElementById("autoseach"),
        ...
        o = i.value;
    if (o.length >= 1 ? i.setAttribute("style", "background: #fff;") : i.setAttribute("style", ""), o.length >= 3)
        if (n.setAttribute("style", "box-shadow: 1px 1px 1px 1px #00000091;"), document.querySelector(".listSearch").classList.add("listSearchA"), t) {
            var r = new FormData;
...
function responsive(e) {
    if (contenedor = document.getElementById("fandomMain"), null != contenedor) {
        var t = document.querySelector(".xBody"),
            i = document.querySelector(".xLat"),
            n = document.querySelector(".xLatSer");
        e.matches ? (null != i && contenedor.appendChild(i), null != t && contenedor.appendChild(t), null != n && contenedor.appendChild(n)) : (null != t && contenedor.appendChild(t), null != i && contenedor.appendChild(i), null != n && contenedor.appendChild(n))
    }
}! function(e, t) {
    ...
    responsive(t), t.addListener(responsive)
});
...
function storeContent(e) {
    idSection = document.getElementById("fandomMain").getAttribute("secid");
...
function corkdw() {
    var e = document.getElementById("fandomCarrusel").getAttribute("items"),
...
function nextC(e, t) {
    ...
    var n = document.getElementById("fandomCarrusel").getAttribute("items"),
    ...
    if (i >= t) {
        ...
        var s = document.getElementById("fandomCarrusel").offsetWidth;
...
function prevC(e, t, n) {
    var o = document.getElementById("fandomCarrusel").getAttribute("items"),
...
up.on("up:fragment:inserted", function(e) {
    ...
    if (null != document.getElementById("fandomCarrusel")) {
        function n(e) {
            e.matches ? (document.getElementById("fandomCarrusel").setAttribute("items", "1"), corkdw()) : (document.getElementById("fandomCarrusel").setAttribute("items", "2"), corkdw())
        }
        document.getElementById("fandomCarrusel").setAttribute("style", "display: block;");
        var o = window.matchMedia("(max-width: 700px)");
        n(o), o.addListener(n);
        var i = document.getElementById("fandomCarrusel").getAttribute("items"),
            s = function() {
                var e = document.getElementById("fandomCarrusel").getAttribute("items");
                carruselfandom(document.getElementById("fandomCarrusel").offsetWidth, e)
            };
        if (window.addEventListener("resize", s), s) carruselfandom(document.getElementById("fandomCarrusel").offsetWidth, i);
        echo.init({
            callback: function(e, t) {}
        })
    }
...
up.on("up:fragment:inserted", function(event) {
	...
	if (input2 != null){
		input2.innerHTML = `<iframe src="https://fandom.co/LAB/native.php" frameborder="0" allowfullscreen="true" webkitallowfullscreen="true" mozallowfullscreen="true"></iframe>`;
	}

});

function idiomaDownload(who, id, ifl) {
      if (id == 1) {
        var remove = document.querySelector(".espa");
        var divCarga = document.querySelector(".espa .intk");
        var divIframe = document.querySelector(".espIframe");
      }if (id == 2) {
        var remove = document.querySelector(".lati");
        var divCarga = document.querySelector(".lati .intk");
        var divIframe = document.querySelector(".latIframe");
      }if (id == 3) {
        var remove = document.querySelector(".subt");
        var divCarga = document.querySelector(".subt .intk");
        var divIframe = document.querySelector(".subIframe");
      }if (id == 4) {
        var remove = document.querySelector(".engl");
        var divCarga = document.querySelector(".engl .intk");
        var divIframe = document.querySelector(".engIframe");
      }
      
      divCarga.innerHTML = `<div class="loadng">${svg_load}</div>`;
      who.classList.add("activeIdioma");
      divIframe.innerHTML = `<div id="ifr_${id}" class="iframeskd"><iframe id="frame_${id}" scrolling="no"></iframe> </div>`;

      setTimeout(function () {
        var myIframe = document.getElementById('frame_'+id);
        myIframe.addEventListener("load", function() {
          
          document.getElementById('ifr_'+id).scrollTop = 585;
...
function removePb(who) {
	who.remove();
	var timep = time(1);
	localStorage.setItem(cdnTerra,timep);
}

up.on("up:fragment:inserted", function(event) {
	if (localStorage.getItem(cdnTerra) != null){
		

		if (time(0) > localStorage.getItem(cdnTerra)) {

			document.querySelector(".cargaAjax").innerHTML = adN1;
...
var timer;
function timedt() {
  timer = setInterval(uno, 1000);
}

var timer2;
function timedt2() {
  timer2 = setInterval(esperadown, 1000);
}

totalTime = 10;

function actionU(who) {

	fetch(URLBASE+'/peticion/check', {
	method: 'GET'
	}) .then(res => res.json())
		.then( data => {	
		if (data.status == 202) {
			who.innerHTML = 'ESPERE...';
			timedt2();
		}
		if (data.status == 200) {
			document.querySelector(".processUser").remove();
		}
		if (data.status == 404) {
			uno('click');
			timedt();
	...
	who.setAttribute('onclick', '');
}

function errorpub() {
	document.querySelector(".msn").innerHTML = '<p>Error, Intentelo mas tarde</p>';
	document.querySelector(".countDownA span").remove();
	document.querySelector(".countDownA").setAttribute('style', 'border: 1px solid #d01c1c;');
	if (timer != null){ 
		clearInterval(timer);
	}if (timer2 != null){ 
		clearInterval(timer2);
...
function uno(action) {
	...
	who = document.querySelector(".countDownA span");

	if(action == 'click') {

		who.innerHTML = 'ESPERANDO ACCIÓN';
		who.setAttribute('style', 'background: #528813;');
		document.querySelector(".countDownA").setAttribute('style', 'border: 1px solid #528813;');
	    document.querySelector(".msn").innerHTML = '';
		// red = window.open(URLBASE+"/action");
		red = window.open(URLBASE+"/action");
		who.removeAttribute('onclick');
	...
	if(action != 'click') {
		...
		if (red.closed) {
			...
			document.querySelector(".msn").innerHTML = '<p>Acción no completada</p>';
			who.innerHTML = 'VOLVER A INTENTAR';
			who.setAttribute('style', 'background: #d01c1c;');
			document.querySelector(".countDownA").setAttribute('style', 'border: 1px solid #d01c1c;');
			who.setAttribute('onclick', 'actionU(this);');
			clearInterval(timer);
			delete totalTime;
			totalTime = 10;
...
function esperadown(){
	if(totalTime==0){
		fetch(URLBASE+'/peticion/final', {
		method: 'GET'
		}) .then(res => res.text())
		.then( data => {}).catch(function(error) { errorpub(); });
		...
		document.querySelector(".countDownA b").innerHTML = totalTime-=1;
...
function S_One(who, id) {
	 document.querySelector(".Lang2Video").classList.add("S_SeA");

	 var n = document.getElementsByClassName("SelectServer");
	 for (i = 0; i < n.length; i++) {
		n[i].classList.remove("S_ServerA");
	 }
	 document.querySelector(".Server_L_"+id).classList.add("S_ServerA");
}

function C_One(who, id, linkT) {

	var str = linkT;
  	var n23 = str.includes('vrs');

	//PUBLICIDAD 
  	if (n23 == true) {

  		if (localStorage.getItem(cdnVast) != null){
	        if (timeVast(0) > localStorage.getItem(cdnVast)) {

	        	if (document.querySelector(".PU_IN a") != null){ 
					document.querySelector(".PU_IN a").remove();
				}

	        }else {
		        if (localStorage.getItem(cdnADS) != null){
					if (time(0) > localStorage.getItem(cdnADS)) {
						document.querySelector(".PU_IN").innerHTML = adN3;
					}
				}else{
					document.querySelector(".PU_IN").innerHTML = adN3;
				}
	...
	var n = document.getElementsByClassName("itemServers");
	for (i = 0; i < n.length; i++) {
		n[i].classList.remove("CO_Active");
	}

	who.classList.add("CO_Active");

	document.querySelector(".VIKDU").innerHTML = `<div class="deletedPub"><i></i><span>Comprobando el estado de los videos</span></div>`;
	document.querySelector(".printRetAll").innerHTML = `<span class="ReturnALL" onclick="backALL(this);"><img src="https://i.imgur.com/CyF9UPO.png"></span>`;

	document.querySelector(".Server_L_"+id).classList.remove("S_ServerA");
	
	document.querySelector(".Server_L_"+id).classList.add("C_SeA");
	document.querySelector(".ViIframe").classList.add("ViIframeA");

    document.querySelector(".INST_IF").innerHTML = `<iframe id="iframeRep" frameborder="0" allowfullscreen="true" webkitallowfullscreen="true" mozallowfullscreen="true"></iframe>`;

    
    if (localStorage.getItem(cdnVast) != null){
        if (timeVast(0) > localStorage.getItem(cdnVast)) {doVast2 = true;}else {doVast2 = true;//here
        }
    }else {
    	doVast2 = true;
    }
    ...
    if (doVast2 == true) {
	    ...
	    var myIframe = document.getElementById('iframeRep');	myIframe.src = linkT;
	    setTimeout(function () {

	        if (null != document.querySelector(".deletedPub")) {
	          		 document.querySelector(".deletedPub").remove();
	        }
    	...
    	setTimeout(function () {
        var myIframe = document.getElementById('iframeRep');
        myIframe.addEventListener("load", function() {
...
function backALL(who) {
	var n = document.getElementsByClassName("SelectServer");
	for (i = 0; i < n.length; i++) {
		n[i].classList.remove("C_SeA");
	}
	var n = document.getElementsByClassName("itemServers");
	for (i = 0; i < n.length; i++) {
		n[i].classList.remove("CO_Active");
	}
	document.querySelector(".ViIframe").classList.remove("ViIframeA");
	document.querySelector("#iframeRep").remove();
	who.remove();
	document.querySelector(".Lang2Video").classList.remove("S_SeA");
	document.querySelector(".VIKDU2").innerHTML = `<div class="deletedPub2"><i></i></div>`;
	setTimeout(function () {

        document.querySelector(".deletedPub2").remove();

    }, 333);
}

function removePTK(who) {
	who.remove();
	var timep = time(1);
	localStorage.setItem(cdnADS,timep);
}

function backIdiomaW() {
	var n = document.getElementsByClassName("SelectServer");
	for (i = 0; i < n.length; i++) {
		n[i].classList.remove("S_ServerA");
	}
